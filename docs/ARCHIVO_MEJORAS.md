# Archivo: análisis de mejoras 2026

> **Documento histórico y congelado. No añadas cosas aquí.** El trabajo
> pendiente vive en `docs/ANALISIS_MEJORAS.md`.
>
> Esto es el análisis completo del 12/06/2026 con sus revisiones sucesivas
> (06/08 y 15/08), tal cual quedó, con la numeración original. Se conserva
> porque explica **por qué** se hizo cada cosa y contra qué se verificó, que es
> lo que no cabe en un backlog. Lo que seguía pendiente el 15/08/2026 se
> reescribió sin numerar en el backlog nuevo.
...
- [x] **30. Doble envío en formularios** — **[15/08/2026]** revisados los 18 componentes con `onSubmit`. La mayoría ya deshabilitaban el botón con su `loading`/`enviando`; los que **no tenían absolutamente nada** eran los cuatro formularios de `/configuracion` (crear y editar delegación, crear y editar usuario): pulsar "Crear" dos veces creaba dos delegaciones. *Fix*: `hooks/use-submit-guard.ts` — un cerrojo en `ref` que se cierra de forma **síncrona** (el `disabled` por estado no cubre el hueco entre el clic y el re-render, que es justo donde cae el doble toque en móvil) más un booleano `enviando` para pintar el botón. `components/configuration/configuration-manager.tsx` tiene el mismo agujero pero es código muerto: no lo importa nadie.
...
- [x] **33. Selección no se limpia al cambiar filtros** — **[15/08/2026]** `transaction-manager.tsx` solo podaba la selección contra los movimientos **ya cargados**. Con la lista paginada (100 por página) eso descartaba en silencio lo seleccionado en páginas que aún no habían llegado: se seleccionaban 40 movimientos, se cambiaba un filtro y "Eliminar" actuaba sobre los que quedaban visibles. *Fix*: al cambiar la firma de los filtros (o la delegación) la selección se vacía entera y se avisa con un toast, en vez de encogerse sin decirlo.
...
- [x] **NUEVO — 38c. [corregido 15/08/2026] Borrar entidades vinculadas: o falla en críptico, o desvincula en silencio** — al arreglar el borrado de contactos apareció el mismo patrón en el resto de entidades con relaciones. Verificadas las FK contra la BBDD en vivo antes de tocar nada, porque el diagnóstico original no era exacto en los dos casos que más importaban:
  - **Categoría** (`movimiento.categoria_id`, `categoria.categoria_padre_id` y `regla.categoria_id` son `NO ACTION`): el diálogo prometía que "las transacciones no se eliminarán, pero perderán su asignación de categoría" y eso **nunca ocurría** — con un solo movimiento detrás el borrado fallaba y lo único que se veía era "Error al eliminar la categoría". *Fix*: `getUsosCategoria()` cuenta movimientos, subcategorías, facturas y reglas antes de preguntar; el diálogo los enseña, explica cuál de ellos bloquea, deshabilita "Eliminar" mientras algo lo impida y ofrece **"Solo desvincular"** (que es lo que aquel texto describía) y **"Desactivar"**.
  - **Cuenta** (`movimiento.cuenta_id` es `ON DELETE CASCADE`, **no** `RESTRICT` como decía el análisis): no falla, **borra los movimientos de verdad**. El diálogo ya ofrecía migrarlos y pedía escribir "ELIMINAR", pero en ningún momento decía cuántos había. *Fix*: se cuenta y se enseña el número antes de la casilla de confirmación, y se avisa cuando no hay otra cuenta a la que moverlos.
  - **Factura** (`movimiento.factura_id` es `SET NULL`): ya estaba resuelto — `delete-factura-dialog.tsx` avisa de cuántos movimientos perderán el vínculo y de que no se borran.
...
- [x] **60n. Fila "Editar" parcialmente cortada al entrar en la tabla de Delegaciones/Usuarios de `/configuracion`** — **[15/08/2026]** *Fix*: los botones "Editar" de ambas tablas pasan a ser solo icono (`Pencil`) por debajo de `sm`, con `aria-label`, así que la columna de acciones cabe en la vista inicial. De paso, **"Eliminar usuario" era un clic único sin confirmación**, pegado al botón de editar en móvil: ahora usa `ConfirmButton` (dos clics) y avisa con un toast si la API falla, en vez de quitar la fila de la lista pasara lo que pasara.
- [x] **61n. Panel "Avisos y tareas" a pantalla completa en móvil** — ya resuelto en `components/avisos/avisos-panel.tsx`: el panel ocupa toda la pantalla por debajo de `sm` (los estilos de tarjeta flotante son `sm:`) y lleva su propia flecha de volver.

### Hallazgos nuevos de la revisión de móvil (15/08/2026)

Segunda pasada, a petición del usuario. Facturas queda fuera (se estaba tocando en paralelo).

- [x] **62n. El menú lateral no se cerraba al elegir una sección en móvil** — `components/sidebar.tsx`: el `Sheet` móvil no estaba controlado, así que al pulsar un elemento de navegación la página cambiaba **detrás** del panel, que seguía tapándola. *Fix*: `Sheet` controlado + `onNavigate` que lo cierra al pulsar un enlace. Cerrar al navegar es lo esperado en móvil: el panel es modal y ocupa el 80% del ancho, no hay nada que ver mientras siga abierto.
- [x] **63n. Sangría excesiva de subcategorías en móvil** — `components/categories/category-list.tsx` usaba `ml-4 + pl-3` (28px) por nivel en todos los breakpoints. En 375px eso se come el sitio del nombre. *Fix*: `ml-1.5 + pl-2` (14px) por debajo de `sm`, sin tocar desktop; el borde punteado sigue marcando el nivel.
- [x] **64n. No se veía qué movimientos tienen factura** — `transaction-row-indicators.tsx`: había indicador de "falta factura" (ámbar) pero ninguno para lo contrario. *Fix*: insignia `Receipt` en azul cuando `movimiento.factura_id` apunta a una factura de la bandeja, del mismo tamaño que el resto (20px, cabe en móvil). Se distingue del indicador de archivo adjunto a propósito: un adjunto es un papel colgado del movimiento, una factura es un registro de la bandeja. La marca manual "falta factura" deja de pintarse cuando ya hay factura vinculada, que se contradecían.
- [x] **65n. `/configuracion` decía "Acceso restringido" a un gestor central antes de dejarle entrar** — `useIsAdmin()` arrancaba en `false` y no exponía estado de carga, así que la comprobación de rol (una consulta a `membresia`) pintaba el mensaje de "no autorizado" en el hueco y luego cargaba la página. *Fix*: `useIsAdminState()` devuelve `{ isAdmin, loading }` y `config-page.tsx` muestra un esqueleto mientras no se sabe. `useIsAdmin()` sigue existiendo para los siete sitios que solo habilitan funciones extra, donde el falso negativo momentáneo no dice nada incorrecto.
...
1. **Lote 1 (seguridad, verificar y cerrar):** 4 (dashboard `/` sin proteger), 6 (validación de ficheros), 8 (xlsx desde CDN), 12b/12c (advisors de Supabase).
2. **Lote 2 (bugs de datos):** 32b (ya corregido, desplegar la migración `057` si no se ha hecho vía MCP) — verificar que no hay más agregados client-side sin paginar en el resto de la app.
3. **Lote 3 (rendimiento):** ✅ 13, 16-18, 21-23 hechos (08/2026); ✅ 24 (virtualización de movimientos) y 26 (Suspense/streaming) hechos (08/2026); 15 verificado y deprioritizado (sin volumen real). Queda **25** (`images.unoptimized`), que sigue siendo una decisión de infra del mantenedor (coste de la Image Optimization API de Vercel y dominios remotos permitidos), no un cambio de código aislado.
4. **Lote 4 (deuda técnica):** 66-71, 74.
5. **Lote 5 (pulido móvil):** ✅ cerrado — 60n y 61n hechos, más 62n–65n de la segunda pasada (15/08/2026).
6. **Lote 6 (borrados en cascada, doble envío y selección):** ✅ cerrado — 38c, 30 y 33 hechos (15/08/2026).
