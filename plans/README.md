...
| [022](022-facturas-por-email-y-lectura-con-ia.md) | Buzón de facturas por delegación (webhook de Resend) + lectura automática con Gemini 3.7 Flash | P2 | L | — | DONE (2026-08-14). `scripts/065` (alias_email, factura.categoria_id, tabla factura_email) aplicada y verificada en vivo el 14-08-2026: las 18 delegaciones tienen alias (`castellon`, `ece`, `madrid`…). Núcleo nuevo en `lib/api/factura-ia.ts`, `lib/api/gemini.ts` y `lib/api/facturas-email.ts`, expuesto en app (`/api/facturas/ia`), API externa (`/api/v1/facturas/{id}/leer-ia` y `/categoria`) y MCP (`leer_factura_con_ia`, `aceptar_categoria_factura`). La categoría nunca se aplica sola: se guarda como sugerencia en `datos_ia` hasta que alguien la acepta. UI: panel de lectura en el sheet, dirección del buzón copiable en la bandeja, selector de categoría en facturas (la deuda que el plan 021 dejó abierta al no existir la columna). Docs: `docs/FACTURAS_EMAIL_IA.md` (operador) y `docs/manual/8.-facturas.md` (usuario, sustituye a la sección "integraciones futuras"). `tsc`/`lint`/`test` (192, +42 nuevos) en verde. **No verificado en vivo**: sin claves de Gemini ni buzón de Resend en este entorno, la extracción real y la recepción de un correo están cubiertas solo por tests de las partes puras (firma svix, parseo de direcciones, normalización de fechas/importes). |
