...
| `leer_factura_con_ia` | Lee el PDF de una factura y rellena proveedor (creándolo si no existe), número, fecha e importe. Solo toca los campos vacíos. |
| `aceptar_categoria_factura` | Aplica la categoría que sugirió la lectura automática (o la que se indique). Es un paso aparte a propósito: la IA nunca categoriza sola. |
...
{% hint style="info" %}
**Lo que NO puede tocar:** importe, fecha, cuenta y delegación de un movimiento. Vienen del banco, y cambiarlos desde fuera sería una forma silenciosa de descuadrar la contabilidad. Tampoco puede dar de alta pagos MCM ni contactos a mano: la única alta automática de contactos es el proveedor que detecta `leer_factura_con_ia` al leer una factura, y queda marcado como creado automáticamente para que se pueda repasar.
{% endhint %}
